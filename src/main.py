...
class GIF:
    """class to create the GIF animation"""

    def __init__(self, track_id):
        self.cnt = 0
        self.name = track_id + 1
        self.frames = []
        self.start()

    def __call__(self):
        if parser.export_gif:
            curr_frame_path = os.path.join(
                'gif', f'gif_{self.name}_{self.cnt}.png')
            pygame.image.save(WIN, curr_frame_path)
            curr_frame = imageio.imread(curr_frame_path)
            self.frames.append(curr_frame)
            os.remove(curr_frame_path)
            self.cnt += 1

        if self.cnt >= 452:
            imageio.mimsave(f'gif_{self.name}.gif',
                            self.frames, format='GIF', duration=0.02)
            sys.exit()
...
def parse_args() -> Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="racing track to be used on the simulation. {Default: None(Random track)}",
    )
    ...
    parser.add_argument(
        ...
        dest='user_test',
        default=False,
        help="spawn car that can be controlled by user. {Default: False}",
    )
    ...
    parser.add_argument(
        ...
        dest='agent_test',
        default=False,
        help="test the simulation with only the best car. {Default: False}",
    )
    ...
    parser.add_argument(
        ...
        dest='show_cleaner',
        default=False,
        help="show the car used to draw the Cleaning line. {Default: False}",
    )
    ...
    parser.add_argument(
        ...
        default=True,
        help="show the population evolving in a simulation. {Default: True}",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="how many cars for each generation. {Default: 100}",
    )
    ...
    parser.add_argument(
        ...
        dest='export_gif',
        default=False,
        help="export gif with agent driving in four tracks + bonus. {Default: False}",
    )
...
def main() -> None:
    ...
    my_gif = GIF(track.track_id)

    if parser.agent_test:
        if not parser.show_cleaner:
            PIDCar = None
    ...
    while True:
        ...
        my_gif()  # create gif if user requested
